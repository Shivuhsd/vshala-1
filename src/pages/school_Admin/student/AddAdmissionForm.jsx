...
//     if (
//       ["name", "father_name", "mother_name"].includes(name) &&
//       /\d/.test(value)
//     )
//       return;
//     if (
//       ["phone", "father_phone", "mother_phone"].includes(name) &&
//       !/^\d{0,10}$/.test(value)
//     )
//       return;
...
//         <button
//           onClick={() => navigate("/school-admin/students/bulk-admission")}
...
//             className="border px-4 py-2 rounded col-span-3"
//           />
//           <input
//             name="city"
//             value={formData.city}
...
//                 <li key={subj.id}>
//                   {subj.label} –{" "}
//                   <span className="text-gray-600">{subj.subject_code}</span>
//                 </li>
//               ))}
//             </ul>
//           </div>
...
//           />
//           {renderFileInput(
//             "Upload Parent Signature (PDF or Image)",
//             "parent_signature"
//           )}
//           {renderFileInput(
//             "Upload Parent ID proff (PDF or Image)",
//             "parent_id_proof"
//           )}
//         </div>
//       </section>
...
//           className="px-6 py-2 bg-[#6B21A8] hover:bg-[#9333EA] text-white rounded-md font-medium"
//         >
//           Submit Admission
//         </button>
//       </div>
...
const SUBCASTE_MAP = {
  GM: ["GM"],
  SC: ["SC", "ST"],
  ST: ["SC", "ST"], // treat SCT same as SC/ST choice per your requirement
  "SC/ST": ["SC", "ST"], // in case backend sends/needs this form
...
const AddAdmissionForm = () => {
    ...
    if (selectedSchool?.id && selectedSession?.id) {
      const fetchClasses = async () => {
        try {
          ...
          // res.data.results is expected: array of class/section links
  ...
  // ---- set sections + fetch subject groups when class changes ----
  useEffect(() => {
    if (formData.class) {
      // Sections: derive from links for the selected class
      const linksForClass = classOptions.filter(
        (cls) => cls.id === formData.class || cls.class_id === formData.class
      );
      // If links item has embedded sections array, use it; else derive from links list
      const embeddedSections = linksForClass[0]?.sections;
      if (Array.isArray(embeddedSections)) {
        setSectionOptions(
          embeddedSections.map((s) => ({
            id: s.id || s.section_id,
            label: s.label || s.section_label || s.name || "Section",
          }))
        );
      } else {
        // derive from links rows that match the chosen class
        const derived = classOptions
          .filter(
            (row) =>
              (row.class_id || row.id) === formData.class ||
              row.id === formData.class
          )
          .map((row) => ({
            id: row.section_id || row.section?.id || row.id, // fallback
            label:
              row.section_label ||
              row.section?.label ||
              row.section?.name ||
        ...
        const dedup = Array.from(
          new Map(derived.map((s) => [s.id, s])).values()
        );
        setSectionOptions(dedup);
      }

      // Subject Groups for class
      const fetchGroups = async () => {
        try {
          const classId =
            linksForClass[0]?.class_id ||
            linksForClass[0]?.id ||
            formData.class;
          const res = await axiosInstance.get(
  ...
  // ---- caste/sub-caste computed options ----
  const availableSubCastes = useMemo(() => {
    if (!formData.caste) return [];
    const key = SUBCASTE_MAP[formData.caste]
      ? formData.caste
      : formData.caste || "";
    return SUBCASTE_MAP[key] || [];
  }, [formData.caste]);

  // If caste changes, reset sub_caste if it's no longer valid
  useEffect(() => {
    if (!availableSubCastes.includes(formData.sub_caste)) {
      setFormData((prev) => ({ ...prev, sub_caste: "" }));
    }
  }, [availableSubCastes]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-xl shadow space-y-10">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setFormData((prev) => ({
                ...prev,
                caste: newCaste,
                sub_caste: SUBCASTE_MAP[newCaste]?.includes(prev.sub_caste)
                  ? prev.sub_caste
                  : "",
              }));
            }}
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            <option value="">
              {formData.class ? "Select Section *" : "Select Class first"}
            </option>
            {sectionOptions.map((sec) => (
              <option key={sec.id} value={sec.id}>
                {sec.label}
          ...
          >
            <option value="">
              {formData.class ? "Select Group *" : "Select Class first"}
            </option>
            {groupOptions.map((grp) => (
              <option key={grp.id} value={grp.id}>
                {grp.label} {grp.code ? `(${grp.code})` : ""}
              </option>
            ))}
          </select>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {renderFileInput(
            "Upload Parent ID proof (PDF or Image)",
            "parent_id_proof"
          )}
        </div>
      </section>
