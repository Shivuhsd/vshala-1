...
const normalizeRow = (raw) => {
  ...
  return { id, name, class_name, section_label, __raw: raw };
};
    ...
    try {
      ...
      const normalized = rawList.map((raw) => {
        ...
        if (rawClassId && classMap[String(rawClassId)]) {
          row.class_name = classMap[String(rawClassId)];
        } else if (row.class_name && classMap[String(row.class_name)]) {
          row.class_name =
            classMap[String(row.class_name)] ?? row.class_name;
        }
